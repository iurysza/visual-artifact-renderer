...
export const artifactManifest = {
  ...
  "execution-trace": {
    ...
    description: "Debugger-style execution review with one persistent, scrollable call stack capped at six visible frames. Previous/next moves the current frame, updates active-line source context, and shows typed inputs/outputs for that frame. Event code may use content directly or a create-time src path that the CLI safely reads and inlines.",
    props: {
      title: "string?",
      caption: "string?",
      ...
      events: "ExecutionTraceEvent[] (1-100; boundary events require boundary.from/to/kind/outcome; values require typed preview + provenance; callStack carries typed frames; code accepts { content?, src?, language? } and requires content or src)",
      initialEventId: "string? (select the most review-relevant event on load)",
        ...
        events: [
          {
            ...
            callStack: [{ id: "safe-parse", fn: "VisualArtifactSpecSchema.safeParse", signature: "safeParse(data: unknown)", returnType: "SafeParseReturnType<VisualArtifactSpec>", active: true }],
          },
        ],
      },
    },
